...
#                 print(self.params)
                ...
                print(self.params['beta' + str(i+1)])
            ...
            if (i == self.num_layers - 2):
                self.params['W' + str(self.num_layers)] = np.random.normal(0, weight_scale, [hidden_dims[i], num_classes])
...
#             print("fc shape", scores_fc.shape)
            if self.normalization=='batchnorm':
                scores_bn, cache_bn[i] = batchnorm_forward(scores_fc, self.params['gamma'+str(i+1)], self.params['beta'+str(i+1)], self.bn_params[i]) #gamma, beta, bn_param):
...
#         print("-----------------------------------------")
        ...
        #先把好算的loss给算了
        loss, dsoftmax = softmax_loss(scores, y)
        loss += 0.5*self.reg*(np.sum(self.params['W'+str(self.num_layers)]*self.params['W'+str(self.num_layers)])) #???为什么这个正则化的误差只要计算最后一层的权重了？之前的那些层不要了吗
#         print(loss)
...
#             print("cache_relu.shape=", cache_relu[i-1].shape)
            ...
            if self.normalization=='batchnorm':
                dbatchnorm, dgamma, dbeta = batchnorm_backward(drelu, cache_bn[i-1])
                drelu = dbatchnorm
...
#             drelu = relu_backward(dx_last, relu_cache[i-1])

#             if self.normalization=='batchnorm':
#                 dbatchnorm, dgamma, dbeta = batchnorm_backward(drelu, bn_cache[str(i)])
#                 dx_last, dw_last, db_last = affine_backward(dbatchnorm, fc_cache[i-1])
#                 grads['beta' + str(i)] = dbeta
#                 grads['gamma' + str(i)] = dgamma
#             else:
